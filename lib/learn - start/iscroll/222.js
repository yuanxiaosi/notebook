(function(w){
	w.IScroll = function(id){
		...
		var scroller = document.querySelector(".ul");
		...
		var maxScrollX = wrapperWidth - scrollerWidth;
		...
		wrapper.addEventListener('touchend', function(e){
			...
			if(moveTime < 150){
				momenValue = momentum(x, transX, moveTime, maxScrollX, wrapperWidth)
			}
		...
		function scrollMove(x){
			scrollerStyle.transform = "translateX("+x+"px)";
		}
		...
		function transEnd(momenValue){
			...
			scrollerStyle.transform = "translateX("+momenValue.destination+"px)";
			setTimeout(function(){
				scrollBack(); //超出回滚
			}, momenValue.duration)
		...
		function scrollBack(){
			...
			if(transX < maxScrollX){
				scrollerStyle.transitionTimingFunction = "cubic-bezier(0.175, 0.885, 0.32, 1.275)";
				scrollerStyle.transitionDuration = "500ms";
				scrollerStyle.transform = "translateX("+ maxScrollX +"px)";
				transX = maxScrollX;
			}
		}
